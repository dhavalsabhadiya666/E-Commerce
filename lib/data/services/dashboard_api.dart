...
class DashboardApi {
  ...
  Future<dynamic> getBanner({@required BuildContext context, FormData data}) async {
    try {
      ApiResponseModel apiResponse = await _client.post('${Endpoints.getBannerImages}', data: data);
  ...
  Future<dynamic> getWallet({@required BuildContext context, FormData data}) async {
    try {
      ApiResponseModel apiResponse = await _client.post('${Endpoints.getWallet}', data: data);
